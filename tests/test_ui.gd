...
func run() -> void:
	...
	view.action("avatar_choice_1")
	check(view.selected_avatar == 1 and view.selected_avatar != view.current_avatar, "Another unlocked avatar can be selected")
	view.action("apply_avatar")
	check(view.current_avatar == 1, "Change avatar applies the selected portrait")
	view.action("avatar_choice_8")
	check(view.selected_avatar == 1, "Locked avatars cannot be selected")
	view.action("close_avatar")
	check(view.modal == "profile", "Avatar picker closes back to the profile panel")
